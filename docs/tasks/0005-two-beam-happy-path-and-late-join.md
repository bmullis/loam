...
Cross-BEAM happy path: start `Loam.Anchor` on two BEAMs with the same `:registry` and `:name`. Exactly one BEAM ends up holding the child; the other observes the existing owner via Registry's snapshot bootstrap and stays in standby. A late-joining third BEAM behaves the same way: snapshot bootstrap shows the existing owner, no spurious start, no spurious eviction.
