...
TLA+ module under `docs/specs/0003-singleton-on-zenoh/` characterizing the singleton's eventual-uniqueness property and the monitor primitive's debounce semantics. Follows the convention established by `docs/specs/0002-registry-on-zenoh/`: module file, at least one model config, README explaining what the spec models and what it deliberately does not.

Two invariants are load-bearing and must be expressed:

- **EventualUniqueness**: `□◇ (∃ at-most-one BEAM owns name)` after network quiescence — the singleton converges to one live owner.
...
- [ ] `docs/specs/0003-singleton-on-zenoh/` directory created with module file, at least one `.cfg` model config, README.
