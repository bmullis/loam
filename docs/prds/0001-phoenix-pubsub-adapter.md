...
2. **Partition semantics are the honest minimum.** Characterize, do not promise. During partition: drop. After heal: resume. No replay, no publication cache, no querying subscriber. This matches BEAM-distribution's existing behavior and avoids opening the CRDT-shaped history question, which is parked in Phase 5+.
...
5. As an application developer, I want to know that during a network partition my in-flight broadcasts are dropped and that delivery resumes cleanly once the partition heals, so that I can reason about my application's behavior under realistic network conditions.
...
4. Scripted partition test, ExUnit-tagged `@tag :partition`, excluded from default `mix test`. Uses `pfctl` on Darwin or `iptables` on Linux to block the Zenoh port for a window. Asserts: messages published before the partition reach the far side; messages published during the partition do not; messages published after heal reach the far side; no process crashes occur. Documented run instructions live in the test module's moduledoc or an adjacent markdown file.
